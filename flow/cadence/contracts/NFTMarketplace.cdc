...
import Fans from 0x50a0fed12f57c962 // First deployed this
...
pub contract NFTMarketplace {
  ...
  pub struct SaleItem {
    ...
    pub let nftRef: &Fans.NFT
    
    init(_price: UFix64, _nftRef: &Fans.NFT) {
      self.price = _price
      self.nftRef = _nftRef
    }
  }
  ...
  pub resource interface SaleCollectionPublic {
    ...
    pub fun purchase(id: UInt64, recipientCollection: &Fans.Collection{NonFungibleToken.CollectionPublic}, payment: @FlowToken.Vault)
  }
  ...
  pub resource SaleCollection: SaleCollectionPublic {
    ...
    pub let FansCollection: Capability<&Fans.Collection>
    ...
    pub fun listForSale(id: UInt64, price: UFix64) {
      pre {
        ...
        self.FansCollection.borrow()!.getIDs().contains(id): "This SaleCollection owner does not have this NFT"
      }
    ...
    pub fun purchase(id: UInt64, recipientCollection: &Fans.Collection{NonFungibleToken.CollectionPublic}, payment: @FlowToken.Vault) {
      ...
      recipientCollection.deposit(token: <- self.FansCollection.borrow()!.withdraw(withdrawID: id))
    ...
    init(_FansCollection: Capability<&Fans.Collection>, _FlowTokenVault: Capability<&FlowToken.Vault{FungibleToken.Receiver}>) {
      self.forSale = {}
      self.FansCollection = _FansCollection
      self.FlowTokenVault = _FlowTokenVault
    }
  }

  pub fun createSaleCollection(FansCollection: Capability<&Fans.Collection>, FlowTokenVault: Capability<&FlowToken.Vault{FungibleToken.Receiver}>): @SaleCollection {
    return <- create SaleCollection(_FansCollection: FansCollection, _FlowTokenVault: FlowTokenVault)
  }
